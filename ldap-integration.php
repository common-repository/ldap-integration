<?php
/*
  Plugin Name: LDAP Integration
  Version: 1.0.0
  Plugin URI: http://alex-dekker.nl/
  Description: Allows WordPress to authenticate users through LDAP
  Author: Alex Dekker
  Author URI: http://alex-dekker.nl/
 */

/* 	Copyright 2013 Alex Dekker (email : alex@alex-dekker.nl)
...
if (!class_exists('LdapIntegrationPlugin')) {

    class LdapIntegrationPlugin {
        var $authenticated = false;
        var $ldap_auth_domain = 'ldap-auth';

        function LdapIntegrationPlugin() {
            ...
            load_plugin_textdomain($this->ldap_auth_domain, PLUGINDIR . '/' . dirname(plugin_basename(__FILE__)) . '/languages', dirname(plugin_basename(__FILE__)) . '/languages');
        }
        /*************************************************************
         * Plugin hooks
         ************************************************************ */
        ...
        */
        function initialize_options() {
            if (current_user_can('manage_options')) {
                add_option('LDAP_integration_auto_create_user', false, 'Should a new user be created automatically if not already in the WordPress database?');
                add_option('LDAP_integration_use_ssl', false, 'Use SSL Connection');
                add_option('LDAP_integration_server', 'localhost', 'LDAP Server');
                add_option('LDAP_integration_base_dn', '', 'Base DN');
                add_option('LDAP_integration_group_base_dn', '', 'Group Base DN');
                add_option('LDAP_integration_role_equivalent_groups', '', 'Role Equivalent Groups');
                add_option('LDAP_integration_default_email_domain', '', 'Default Email Domain');
                add_option('LDAP_integration_bind_dn', '', 'Bind DN');
                add_option('LDAP_integration_bind_password', '', 'Bind Password');
                add_option('LDAP_integration_uid_filter', '(uid=%user_id%)', 'LDAP uid search filter');
                add_option('LDAP_integration_group_filter', '(cn=%group%)', 'LDAP group search filter');
                add_option('LDAP_integration_group_attribute', 'memberuid', 'LDAP group attribute');
            }
        }
        
        /*
         * Search ldap on login!!
         */
        function authenticate( $username, $password ) {
            $this->authenticated = false;
            
            $ldap_server = get_option('LDAP_integration_server');
            $use_ssl = get_option('LDAP_integration_use_ssl');
            $base_dn = get_option('LDAP_integration_base_dn');
            $bind_dn = get_option('LDAP_integration_bind_dn');
            $bind_password = get_option('LDAP_integration_bind_password');
            $uid_filter = get_option('LDAP_integration_uid_filter');
            $replace_count = 0;
            ...
            if ($ldap_bind) {
                ...
                $user_role = $this->_get_user_role($ldap, $ldap_user);
                if ( ($user_role == '') && defined('WP_DEBUG') && ( true === WP_DEBUG ) )
                {
                    trigger_error('LDAP user: \'' . $ldap_user . '\' is not authorized for this site.');
                    return false;
                } elseif ( ($user_role == '') && defined('WP_DEBUG') && ( false === WP_DEBUG ) ) {
                    return false;
                } 
                ...
                if ($user) {
                    $this->_update_user_password($username, $this->_get_password());
                }
                
                if (!$user or $user->user_login != $username) {
                    if ((bool) get_option('LDAP_integration_auto_create_user')) {
                        $sn_lang = 'sn;lang-' . WPLANG;
                        $gn_lang = 'givenname;lang-' . WPLANG;
                        $result = @ldap_search($ldap, $base_dn, $uid_filter, array('sn', 'givenname', 'mail', $sn_lang, $gn_lang)
                        );
         ...
         */
        function _user_in_group($ldap, $ldap_user, $group) {
            $base_dn = get_option('LDAP_integration_group_base_dn');
            $group_filter = get_option('LDAP_integration_group_filter');
            $group_attr = strtolower(get_option('LDAP_integration_group_attribute'));
            $replace_count = 0;
            ...
            return in_array($ldap_user, $members);
        }
        
        function _get_user_role($ldap, $ldap_user) {
            $role_equiv_groups = get_option('LDAP_integration_role_equivalent_groups');
            ...
            foreach ($role_equiv_groups as $role_group) {
                ...
                if ($this->_user_in_group($ldap, $ldap_user, $ldap_group)) {
            ...
            if (defined('WP_DEBUG') && ( true === WP_DEBUG )) {
                if ($user_role != '')
                    trigger_error('User \'' . $ldap_user . '\' is assigned as \'' . $user_role . '\' role.');
                else
                    trigger_error('User \'' . $ldap_user . '\' is not assigned to any role.');
            }
        ...
        function _create_user($username, $email, $first_name, $last_name, $role = '') {
            ...
            $email_domain = get_option('LDAP_integration_default_email_domain');
        ...
        function _update_user($username, $email, $first_name, $last_name, $role = '') {
            ...
            if (!$user_id) {
                ...
                update_user_meta($user_id, 'first_name', $first_name);
                update_user_meta($user_id, 'last_name', $last_name);
                update_user_meta($user_id, 'user_pass', md5($password));
                wp_update_user(array('ID' => $user_id, 'user_pass' => md5($password)));
                if ($role != '')
                    wp_update_user(array('ID' => $user_id, 'role' => $role));
            }
        }
        ...
        function add_options_page() {
            if (function_exists('add_options_page')) {
                $page = add_options_page(__('LDAP Integration', $this->ldap_auth_domain), __('LDAP Integration', $this->ldap_auth_domain), 9, $this->ldap_auth_domain, array(&$this, '_display_options_page'));
        ...
        function _reset_options() {
            $all_options = array_filter(array_keys((array) get_alloptions()), create_function('$target', 'return ereg("^LDAP_integration_", $target);'));
        ...
        function _display_options_page() {
            ...
            $use_ssl = get_option('LDAP_integration_use_ssl');
            $base_dn = get_option('LDAP_integration_base_dn');
            $group_base_dn = get_option('LDAP_integration_group_base_dn');
            $role_equiv_groups = get_option('LDAP_integration_role_equivalent_groups');
            $auto_create_user = (bool) get_option('LDAP_integration_auto_create_user');
            $email_domain = get_option('LDAP_integration_default_email_domain');
            $bind_dn = get_option('LDAP_integration_bind_dn');
            $bind_password = get_option('LDAP_integration_bind_password');
            $uid_filter = get_option('LDAP_integration_uid_filter');
            $group_filter = get_option('LDAP_integration_group_filter');
            $group_attr = get_option('LDAP_integration_group_attribute');
            ?>
...
<div class="wrap">
	...
	<form action="options.php" method="post">
		...
		<input type="hidden" name="page_options" value="LDAP_integration_group_base_dn,LDAP_integration_auto_create_user,LDAP_integration_base_dn,LDAP_integration_server,LDAP_integration_use_ssl,LDAP_integration_role_equivalent_groups,LDAP_integration_default_email_domain,LDAP_integration_bind_dn,LDAP_integration_bind_password,LDAP_integration_uid_filter,LDAP_integration_group_filter,LDAP_integration_group_attribute" />
		<?php if (function_exists('wp_nonce_field')): wp_nonce_field('update-options'); endif; ?>

		<table class="form-table">
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_server"><?php _e('LDAP Server', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_server" id="LDAP_integration_server" value="<?php echo $ldap_server; ?>" /><br />
					<?php _e('LDAP Server (e.g. <code>ldap.example.net</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
						  <th scope="row"><label for="LDAP_integration_use_ssl"><?php _e('Use SSL connection with LDAP?', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="checkbox" name="LDAP_integration_use_ssl" id="LDAP_integration_use_ssl"<?php if ($use_ssl) echo ' checked="checked"' ?> value="1" /><br />
					<?php _e('If you use SSL connection or not, when LDAP connection.', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_base_dn"><?php _e('Base DN', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_base_dn" id="LDAP_integration_base_dn" value="<?php echo $base_dn; ?>" /><br />
					<?php _e('Base DN (e.g., <code>dc=example,dc=net</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
                        
                        <tr valign="top">
				<th scope="row"><label for="LDAP_integration_group_base_dn"><?php _e('Group Base DN', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_group_base_dn" id="LDAP_integration_group_base_dn" value="<?php echo $group_base_dn; ?>" /><br />
					<?php _e('Group Base DN (e.g., <code>cn=role,ou=roles,o=site,dc=example,dc=net</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
                        
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_bind_dn"><?php _e('Bind DN', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_bind_dn" id="LDAP_integration_bind_dn" value="<?php echo $bind_dn; ?>" /><br />
					<?php _e('Bind DN (e.g., <code>cn=proxyuser,dc=example,dc=net</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_bind_password"><?php _e('Bind Password', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="password" name="LDAP_integration_bind_password" id="LDAP_integration_bind_password" value="<?php echo $bind_password; ?>" /><br />
					<?php _e('Password for database login account.', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_uid_filter"><?php _e('User ID Search Filter', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_uid_filter" id="LDAP_integration_uid_filter" value="<?php echo $uid_filter; ?>" /><br />
...
This setting must contain <code>%user_id%</code> string.', $this->ldap_auth_domain); ?>
			...
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_group_filter"><?php _e('Group Search Filter', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_group_filter" id="LDAP_integration_group_filter" value="<?php echo $group_filter; ?>" /><br />
...
This setting must contain <code>%group%</code> string.', $this->ldap_auth_domain); ?>
			...
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_group_attribute"><?php _e('Group Member Attribute', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_group_attribute" id="LDAP_integration_group_attribute" value="<?php echo $group_attr; ?>" /><br />
					<?php _e('LDAP attribute for group member (e.g., <code>memberuid</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_auto_create_user"><?php _e('Automatically create accounts for any and all users can authenticate to the LDAP?', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="checkbox" name="LDAP_integration_auto_create_user" id="LDAP_integration_auto_create_user"<?php if ( $auto_create_user ) echo ' checked="checked"' ?> value="1" /><br />
...
<strong>Users with role equivalent groups will be created even if this setting is turned off</strong> (because if you didn\'t want this to happen, you would leave that option blank.)'); ?>
			...
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_default_email_domain"><?php _e('Default email domain', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_default_email_domain" id="LDAP_integration_default_email_domain" value="<?php echo $email_domain; ?>" /><br />
			...
			<tr valign="top">
				<th scope="row"><label for="LDAP_integration_role_equivalent_groups"><?php _e('Role Equivalent Groups', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_integration_role_equivalent_groups" id="LDAP_integration_role_equivalent_groups" value="<?php echo $role_equiv_groups; ?>" /><br />
...
$LdapIntegrationPlugin = new LdapIntegrationPlugin();
?>
